...
These methods are designed to behave quietly on non-interval inputs. 

Interval to float methods, IR -> R:
An Interval object is expected as input. 
If a non-interval is passed rules for equivalent non-degenerate intervals will apply.

Interval to bool methods, IR -> {0,1}:
This category of methods input an interval and return a True/False literal. 
If a non-interval is passed rules for equivalent non-degenerate intervals will apply.
...
import numpy
from numpy import (ndarray,asarray,transpose,vstack,linspace,zeros)
...
def split_interval(x:Interval,y:float):
...
def subtile(x_:Interval, n:Union[int,tuple]=0):
    x = intervalise(x_)
    d = len(x.shape)
    if n==0: return x
...
def subtiling_to_interval(x_:Interval):
    x = intervalise(x_)
    d = len(x.shape) # dimension of the subtiling ==1 if scalar, ==2 if 1d array
    if d==1: return Interval(lo=min(x.lo), hi=max(x.hi))
